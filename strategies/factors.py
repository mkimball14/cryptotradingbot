...
def calculate_relative_strength(close_prices, window=30, benchmark_symbol='BTC-USD'):
    ...
    try:
        ...
        # Create a DataFrame with benchmark returns repeated for each date
        benchmark_df = pd.DataFrame(index=returns.index)
        
        # Add benchmark returns for each date (repeating across all symbols)
        for date in benchmark_returns.index:
            idx = pd.IndexSlice
            benchmark_df.loc[idx[:, date], 'benchmark_return'] = benchmark_returns.loc[date]
        
        # Calculate relative strength (asset return - benchmark return)
        # Adding 1 to both sides to get a ratio (>1 means outperforming)
        relative_strength = (1 + returns) / (1 + benchmark_df['benchmark_return'])
